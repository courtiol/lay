...
We used **mutate()** from **{dplyr}** to create a new column called *everused*, and we used **pick()** from that same package to remove the column *caseid* when laying down each row of the data and applying the function **any()**.

When combining **lay()** and **{dplyr}**, you should always use **pick()** or **across()**. The functions **pick()** and **across()** let you pick among many [selection helpers](https://tidyselect.r-lib.org/reference/language.html) from the package **{tidyselect}**, which makes it easy to specify which columns to consider.
